...
if (sprite_exists(portrait_sprite)) {
	...
	draw_sprite(sPortrait, 0, draw_portrait_x, draw_portrait_y);
...
if (speaker_name != "") {
	...
	draw_sprite_stretched(sName, 0, x + speaker_x, y + speaker_y - speaker_height / 2, name_w, speaker_height);
...
if (finished && option_count > 0) {
	...
	draw_set_colour(option_text_color);
	for (var i = 0; i < option_count; i++) {
		...
		if (i == current_option) {
			...
			draw_sprite(sOptionArrow, 0, opt_x, opt_y);
		}
		
		draw_sprite_stretched(sOption, 0, opt_x, opt_y - option_height / 2, option_width, option_height);
